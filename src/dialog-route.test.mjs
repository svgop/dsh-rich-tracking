...
test('scout delivers the ONE-subagent queue brief through the real route', async () => {
  ...
  assert.equal((text.match(/TASK — research this row two-sided/g) ?? []).length, 3, 'every lane payload is self-contained')
  assert.match(text, /A\. INSIDE the system: read the code, logic, and systems this row touches/)
  assert.match(text, /B\. OUTSIDE the system: study 3-6 competitors/)
  assert.match(text, /C\. CONCLUDE against both sides/)
