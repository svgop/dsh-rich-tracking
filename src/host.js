...
Writing standard (every label, note, evidence, detail, and item — the operator reads the board cold, mid-context-switch): write for a capable engineer who has never seen this project; the first read must land. Labels are plain noun phrases naming the outcome or component ("Fix login retry loop", where "auth-observability uplift" says nothing). Notes are one sentence with subject and verb, carrying the concrete change and its number — "Replaced the retry loop in login.ts; failed logins 4.1% to 0.2% in staging" lands, "progressed orchestration seams per plan" is noise. Evidence names the artifact and the count. Detail is a status report for someone new to the project: short paragraphs or one-fact bullets, every abbreviation spelled out at first use, plain words over coined jargon and noun-chains. The boring exact sentence beats the clever vague one.
...
WRITING STANDARD (labels, notes, evidence, details, items — everything the operator reads): write for a reader with zero context. Plain technical English; complete short sentences with subject and verb; active voice; one fact per sentence; the point first. A label names a checkable outcome; a note says what changed as a full sentence; evidence names real artifacts with counts; detail opens with the current state in one line, then what remains, then decisions. Spell a codename out where you name it, and let numbers and file paths carry the weight. Exemplars — label: "Rotate auth tokens every 30 days" · note: "Rotation is implemented and tested; the scheduler is still pending." · evidence: "docs/plan/auth.md items 3/7 checked; auth.spec.ts 12/12 pass" · detail opening: "Current state: rotation works but runs manually. Remaining: scheduling and alerts. Decided: 30-day window over 90-day for compliance."
...
function trackingWriteTool() {
  const rowSchema = {
    ...
    properties: {
      ...
      label: { type: 'string', description: 'Row name shown on the board, <= 80 chars. A plain noun phrase naming the outcome or component, parsed instantly by someone who has never seen this project ("Fix login retry loop").' },
      ...
      note: { type: 'string', description: '<= 400 chars: one sentence with subject and verb, the concrete change and its number ("Replaced the retry loop in login.ts; failed logins 4.1% to 0.2% in staging") — or, for blocked rows, the blocker. This is the last write, not a slogan.' },
      ...
      items: {
        ...
        items: {
          ...
          additionalProperties: false,
          properties: {
            label: { type: 'string', description: 'One acceptance item, <= 240 chars: a checkable fact in plain words ("engine tests green", "login retry loop replaced in login.ts").' },
      ...
      detail: {
        type: 'string',
        description: "Optional, <= 4000 chars: the row's full record for the operator's '?' dialog — summary of what is done, details of what still to do, key decisions, open questions, scouted competitive knowledge. Plain text/markdown, written as a status report for an engineer new to the project: short paragraphs or one-fact bullets, abbreviations spelled out at first use, plain words over coined jargon. Keep it current whenever the row's story changes; the board note stays the one-line status, the detail narrates the row.",
      },
      sources: {
        type: 'array',
        maxItems: 12,
      ...
      refs: {
        type: 'array',
        maxItems: 12,
        description: "Optional, up to 12 EXTERNAL references proving the decision/direction — competitor pages, dependency or RFC docs, prior art. Anything outside this repo/workspace belongs here (internal receipts and code paths go in sources); rendered in the row's '?' dialog as the 'why' behind the row.",
        items: { type: 'string', description: 'One external URL or citation, <= 300 chars.' },
      },
    },
  }
  return {
    ...
    description: [
      ...
      'WRITING (the board is read cold, by someone who has never seen this project — every label, note, evidence, detail, item): label = a plain noun phrase a stranger parses instantly ("Fix login retry loop"). note = one sentence with subject and verb, the concrete change and its number ("Replaced the retry loop in login.ts; failed logins 4.1% to 0.2% in staging" — where "progressed orchestration seams per plan" carries zero information). evidence = artifact path plus what was checked, with counts. detail = status-report prose: short paragraphs or one-fact bullets, abbreviations spelled out at first use, plain words instead of coined jargon or noun-chains. The boring exact sentence beats the clever vague one.',
      'percent is derived from artifact truth — the fraction of the row acceptance items (plan checkboxes, landed receipts, verified boxes) that hold right now, never an impression. Overall completion is item-weighted: each item one unit, itemless rows contribute their percent as one unit. items render as the expandable acceptance checklist; detail and sources fill the operator question dialog — keep them current. All operator-facing prose (label, note, evidence, detail, items) follows the WRITING STANDARD in the system section: zero-context reader, complete short sentences, one fact per sentence, checkable outcomes. Calling this after a dismissal re-opens the board.',
    ].join(' '),
    parameters: {
      type: 'object',
      required: ['rows'],
