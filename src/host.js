...
Writing standard (every label, note, evidence, detail, and item — the operator reads the board cold, mid-context-switch): write for a capable engineer who has never seen this project; the first read must land. Plain technical English; complete short sentences with subject and verb; active voice; one fact per sentence; the point first. Labels are plain noun phrases naming the checkable outcome or component ("Fix login retry loop" — where "auth-observability uplift" says nothing). Notes are one sentence carrying the concrete change and its number ("Replaced the retry loop in login.ts; failed logins 4.1% to 0.2% in staging" lands; "progressed orchestration seams per plan" is noise). Evidence names the artifact and the count ("docs/plan/auth.md items 3/7 checked; auth.spec.ts 12/12 pass"). Detail opens with the current state in one line, then what remains, then decisions — short paragraphs or one-fact bullets, abbreviations spelled out at first use, plain words over coined jargon and noun-chains. The boring exact sentence beats the clever vague one.
...
Operator actions land as instructions naming the row: PURSUE (make it the next focus), ALIGN (a VERIFICATION AUDIT: open every row's evidence artifacts, recompute percent from what holds today — correcting DOWN as readily as up — refresh evidence strings, prune dead rows, add missing ones, and land the audit verdict in the board note: survived/corrected/dropped/added/unreadable), DELEGATE (hand the row to a background subagent with a self-contained brief; it tracks its own board in its session; fold its receipts back into the row), SCOUT (ONE continuable background research subagent for ALL open rows, two-sided per row: INSIDE — read the code, logic, dependencies, and packages the row touches, how the existing system handles it today, what to reuse or align with, plus defects and improvement opportunities the reading reveals; OUTSIDE — 3-6 competitors or comparable implementations, approach, key tradeoff, what they got right; then CONCLUDE — adopt what outside proves, keep what our logic already does better, name the concrete alignment or change; lane 1 is the launch prompt; every further row is immediately queued into the SAME agent with send_message — the messages land in its chat as context at once, it makes a todo list of the lanes and works them one at a time in order, returning each result as it lands; findings CONDENSED into durable digests .docs/digest/ .docs/research/, then rows enriched with detail + sources + refs; research is context, not progress: bump a percent only when artifact truth actually changed).
